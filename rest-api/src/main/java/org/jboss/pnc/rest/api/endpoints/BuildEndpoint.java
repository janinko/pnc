...
public interface BuildEndpoint{
    ...
    @Operation(summary = "Push build to Brew.",
            responses = {
                @ApiResponse(responseCode = SUCCESS_CODE, description = "Map of all requested Build ids with boolean status.",
                    content = @Content(array = @ArraySchema( schema = @Schema(implementation = DirStateFactory.Result.class)))), // TODO ??????
                @ApiResponse(responseCode = INVALID_CODE, description = INVALID_DESCRIPTION,
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    ...
    @GET
    @Path("/{id}/brew-push")
    Response get(@Parameter(description = B_ID) @PathParam("id") int id);
